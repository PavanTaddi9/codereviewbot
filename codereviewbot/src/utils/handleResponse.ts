...
export async function handleCommandResponse(
   ...
   greetMessage.setSender(sender).setRoom(room).setGroupable(false).setText(`\`/${command}\` ${args}`)
   const threadID = await modify.getCreator().finish(greetMessage)

   const progressIndicators = [":hammer:", ":hammer_pick:", ":tools:", ":pick:"]

   const builder = modify.getCreator().startMessage()
   builder.setSender(user).setRoom(room).setThreadId(threadID).setText(":pick: Working")
   ...
   const progressIndicatorInterval = setInterval(async () => {
      ...
      updater.setEditor(user).setRoom(room).setText(`${progressIndicators[progressCount++]} Working`)
